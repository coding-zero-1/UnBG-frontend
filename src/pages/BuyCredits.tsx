    ...
    <div className="min-h-[80vh] text-center pt-5 mb-10">
      ...
      <div className="flex flex-wrap justify-center gap-6 text-left">
        {plans.map((elem,index)=>(
          <div key={index} className="bg-white drop-shadow-sm border border-gray-700 rounded-lg py-12 px-8 text-gray-700 hover:scale-105 transition-all duration-500">
            <img width={40} src={assets.logo_icon} alt="" />
            <p className="mt-3 font-semibold">{elem.id}</p>
            <p className="text-sm">{elem.desc}</p>
            <p className="mt-6">
              <span className="text-3xl font-medium">${elem.price}</span> / {elem.credits} Credits
            </p>
